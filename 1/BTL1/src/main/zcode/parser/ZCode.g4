...
keyword_var: (NUMBER | BOOL | STRING) ID (LBRACKET list_NUMBER_LIT RBRACKET)? (ASSIGNINIT exp)?; 
...
if_stmt: IF LPARENT exp RPARENT ignore? list_stmts ignore? list_elif_stmts? else_stmt?;
list_elif_stmts: (elif_stmt list_elif_stmts) | elif_stmt;
elif_stmt: ELIF LPARENT exp RPARENT ignore? list_stmts ignore?;
else_stmt: ELSE ignore? list_stmts ignore?;

assignment_stmt: (ID | index_operators1) ASSIGNINIT exp ignore;
index_operators: (ID | call_stmt1) LBRACKET list_expr RBRACKET;
